...
assert.match(app,/Mio V31[1-9] \(.*\)/,'Actual app release label')
...
has(app,'onEditMatter={matterId=>','Shared matter editor callback is passed')
has(ui,'onAction,onMatterStatus,onEditMatter,getPeople,','Status and matter-editor callbacks are declared alongside post-V307 document-source props')
has(ui,'>Edit matter</button>','Withdrawal workflow rows expose the shared matter editor')
...
has(ui,'const graphFinance=matchingFinance','Graph plots every matching matter, not a capped subset')
assert.ok(!ui.includes('slice(0,12)'),'No fixed cap may hide matching matters from the graph')
