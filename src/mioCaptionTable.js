import { generatedCaseCaption, resolveCaseStyle } from './mioDraftingComponents.js'
...
export function captionModel(data = {}, profile = {}, placeholder = false) {
  const values = placeholder ? {case_style_id:data.case_style_id || '@divorce', petitioner_name:'[Petitioner name]', respondent_name:'[Respondent name]', children:[{name:'[Child / children names]'}]} : data
  const style = resolveCaseStyle(profile, values.matter_case_type || values.case_type || '', !!values.children?.length, values.case_style_id || '')
...
export function expandCaptionMarkers(doc,data,profile={}) {
  for(const p of Array.from(doc.getElementsByTagNameNS(WORD_NS,'p'))){
    ...
    if(table&&cell&&Array.from(table.getElementsByTagNameNS(WORD_NS,'tc')).length>=3){
