...
export function CaptionPreview({styleId,profile}){
 const model=captionModel({case_style_id:styleId},profile,true)
 const display=text=>text.split(/(\[[^\]]+\])/).map((part,i)=>part.startsWith('[')?<Field key={i} label={part.slice(1,-1)}/>:part)
 return <table aria-label="Caption fields" style={{width:'100%',tableLayout:'fixed',borderCollapse:'collapse',fontFamily:'Times New Roman,serif',fontSize:14,lineHeight:1.15,whiteSpace:'normal',textIndent:0}}><colgroup><col style={{width:'51%'}}/><col style={{width:'4.5%'}}/><col style={{width:'44.5%'}}/></colgroup><tbody><tr>
...
export function TemplateParagraph({paragraph,template,file,document,profile,sourceLabel,onEdit}){
 ...
 const styleId=template?.drafting_components_by_file?.[String(file?.id||file?.name||'')]?.case_style_id||template?.default_case_style_id||'@divorce'
 ...
 for(const range of ranges){
  ...
  if(block)pieces.push(<span key={b.id+'-'+range.start} data-mio-source-start={range.start} data-mio-source-end={range.end} data-mio-field-id={b.id} contentEditable={false}>{range.first!==false&&<BlockPreview type={block} profile={profile} styleId={styleId}/>}</span>)
   ...
   const label=kind==='pronoun'?words(b.linked_party||'Client')+' '+words(b.grammar_role||'subject')+' Pronoun':source||b.label||words(b.field_key)
