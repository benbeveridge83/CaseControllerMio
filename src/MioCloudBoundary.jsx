...
export default function MioCloudBoundary({children}) {
  ...
  return <>{waiting&&<main style={panel}><h1>Preparing your Mio workspace</h1><p>{status.phase==='preserving'?'Preserving unsaved edits before reloading.':'Cloud state is loaded. Waiting for workspace initialization before enabling changes.'}</p><button disabled={busy||!!status.pending||!!status.pausedPending} onClick={()=>window.location.reload()}>Retry loading</button></main>}<div data-mio-cloud-phase={status.phase} data-mio-cloud-pending={status.pending} key={boot.user?.id||'login'} style={waiting?{display:'none'}:undefined}>{children}</div>{boot.user&&!waiting&&<MioCloudStatus status={status} busy={busy} onRetry={()=>void store.flushAll()} onPreserve={preserve} onUseCloud={useCloud} onReload={reload}/>}</>
}
