...
export default function MioCloudBoundary({children}) {
  ...
  useEffect(()=>{
    ...
    void supabase.auth.getSession().then(({data,error})=>{if(error)throw error;if(alive)void change(data?.session)}).catch(error=>{if(alive)setBoot({loading:false,user:null,error:error.message||String(error)})})
    const {data}=supabase.auth.onAuthStateChange((event,session)=>setTimeout(()=>{if(alive)void change(session)},0))
    return()=>{alive=false;user.current=undefined;latest.current++;data?.subscription?.unsubscribe()}
  },[])
  ...
  if(boot.loading)return <main style={panel}><h1>Loading Mio from Supabase</h1><p>Cloud data must load before edits are enabled.</p><button onClick={()=>window.location.reload()}>Retry loading</button></main>
  ...
  if(boot.error)return <main style={panel}><h1>Cloud data could not be loaded</h1><p role="alert">{boot.error}</p><p>No browser records were deleted and no empty defaults replaced your cloud data.</p><button onClick={()=>window.location.reload()}>Retry</button></main>
