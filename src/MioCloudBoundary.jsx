...
export default function MioCloudBoundary({children}) {
  ...
  if(boot.loading)return <main style={panel}><h1>Loading Mio from Supabase</h1><p>Cloud data must load before edits are enabled.</p><p role="status">{status.loadProgress?.phase==='reading' ? 'Loaded '+status.loadProgress.loaded+' of '+status.loadProgress.total+' saved records. Large records are downloaded in small pieces.' : status.loadProgress ? 'Reading the saved-record list from Supabase...' : 'Checking your sign-in session...'}</p><small>Cloud startup repair 297</small><br /><button onClick={()=>setRetryAttempt(value=>value+1)}>Retry loading</button></main>
