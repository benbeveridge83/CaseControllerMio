...
const postedRows = `    // A transaction recorded through the classification workflow appears exactly once: as the
    // posted ledger entry. The derived LawPay row is suppressed for the same provider
    // transaction, and a reversed entry contributes nothing.
    const postedTrustRows=(lawPayV323Review.ledger_entries||[])
      .filter(entry=>String(entry.matter_id||'')===matterId&&String(entry.account_key||'').toLowerCase().includes('trust')&&!(lawPayV323Review.ledger_entries||[]).some(other=>String(other.reverses_entry_id||'')===String(entry.id)))
      .map(entry=>{
        ...
        return {id:\`lawpay-entry:\${entry.id}\`,matter_id:matterId,date:financeDateOnly(entry.occurred_at||entry.created_at),occurred_at:entry.occurred_at||entry.created_at,created_at:entry.created_at||entry.occurred_at,direction:entry.direction==='out'?'out':'in',transaction_type:entry.direction==='out'?'refund':'lawpay',amount:Math.abs(financeNumber(entry.amount_cents)/100),payer_payee:provider.payer_name||provider.payer_email||'',reference:provider.reference||'',memo:\`\${entry.direction==='out'?'Refund':'Digital payment'} through LawPay, recorded from the classification review\`,source:'LawPay',lawpay_transaction_id:String(provider.id||provider.gateway_transaction_id||''),lawpay_ledger_entry_id:String(entry.id),lawpay_classification_id:String(entry.classification_id||'')}
      })
