...
export async function askClaudeParalegal({apiKey, gatewayKey, fetchImpl = fetch, message, history = [], snapshot = [], model = DEFAULT_MODEL}) {
  const directKey = String(apiKey || '').trim()
  const gatewayToken = String(gatewayKey || '').trim()
  const question = String(message || '').trim()
  if (!gatewayToken && !directKey) throw Object.assign(new Error('Claude is not configured for this preview.'), {code:'CLAUDE_NOT_CONFIGURED'})
  ...
  const endpoint = useGateway ? 'https://ai-gateway.vercel.sh/v1/messages' : 'https://api.anthropic.com/v1/messages'
  const headers = useGateway
    ? {'content-type':'application/json',Authorization:`Bearer ${gatewayToken}`}
    : {'content-type':'application/json','x-api-key':directKey,'anthropic-version':'2023-06-01'}
  const requestModel = useGateway && !String(model).includes('/') ? `anthropic/${model}` : model
  const response = await fetchImpl(endpoint, {
    method:'POST',
    headers,
    body:JSON.stringify({model:requestModel, max_tokens:1200, system, messages}),
  ...
  return {text, model:data.model || requestModel, usage:{inputTokens:Number(data.usage?.input_tokens)||0, outputTokens:Number(data.usage?.output_tokens)||0}, provider:useGateway?'vercel-ai-gateway':'anthropic'}
}
