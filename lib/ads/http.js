...
export async function handleAdsWorkspace(req,user,base){
 ...
 const posts=['workspace_apply','workspace_suggest','workspace_verify'];if(!['workspace_snapshot','workspace_audience','workspace_history','workspace_compare','keyword_lab_snapshot','keyword_experiments',...posts].includes(action))throw fail('Unknown workspace action.',404)
 ...
 const db={history:()=>rest(`${path}&order=created_at.desc&limit=100`),keywordExperiments:()=>rest(`mio_ads_keyword_experiments?account_id=eq.${encodeURIComponent(accountId)}&order=updated_at.desc&limit=1000`),byRequest:async id=>(await rest(`${path}&request_key=eq.${encodeURIComponent(id)}&limit=1`))?.[0],get:async id=>(await rest(`${path}&id=eq.${encodeURIComponent(String(id))}&limit=1`))?.[0],update:(id,patch)=>rest(`${path}&id=eq.${encodeURIComponent(id)}`,{method:'PATCH',body:patch,headers:{Prefer:'return=representation'}}),reserve:async entry=>{const rows=await rest('mio_ads_changes?on_conflict=account_id,request_key',{method:'POST',body:entry,headers:{Prefer:'resolution=ignore-duplicates,return=representation'}});if(rows?.length)return{fresh:true,entry:rows[0]};const previous=await db.byRequest(entry.request_key);if(!previous)throw fail('Could not reserve approval safely.',503);return{fresh:false,entry:previous}}}
