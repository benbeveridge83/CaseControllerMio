...
import {createKeywordWriteService,keywordRevisionOf} from './keyword-writes.js'
import {adDraft,normalizeAdDraft,requireId,prepareNegatives,aggregateMetrics,aggregateReportingMetrics,reportingMetrics,text,coveringNegative} from './model.js'
import {classifyQuality,effectiveDays,groupTermsByKeyword,normalizeDateRange,withPerDayMetrics} from './keyword-lab.js'
...
function normalizeExperiment(row={}){
 return{id:row.id,campaignId:String(row.campaign_id??row.campaignId??''),campaignName:row.campaign_name??row.campaignName??'',adGroupId:String(row.ad_group_id??row.adGroupId??''),adGroupName:row.ad_group_name??row.adGroupName??'',criterionId:row.criterion_id??row.criterionId??null,criterionResourceName:row.criterion_resource_name??row.criterionResourceName??null,keyword:row.keyword??'',matchType:row.match_type??row.matchType??'',source:row.source??'',hypothesis:row.hypothesis??null,experimentStartedAt:row.experiment_started_at??row.experimentStartedAt??null,approvedBy:row.approved_by??row.approvedBy??null,updatedAt:row.updated_at??row.updatedAt??null,state:row.state??''}
}
function normalizeClassification(row={}){
 return{id:row.id,accountId:String(row.account_id??row.accountId??''),campaignId:String(row.campaign_id??row.campaignId??''),adGroupId:String(row.ad_group_id??row.adGroupId??''),searchTerm:row.search_term??row.searchTerm??'',classification:row.classification??null,note:row.note??null,createdBy:row.created_by??row.createdBy??null,createdAt:row.created_at??row.createdAt??null,updatedAt:row.updated_at??row.updatedAt??null}
}
const classificationIdentity=(row,defaultAccountId='')=>JSON.stringify([String(row.accountId??defaultAccountId),String(row.campaignId??''),String(row.adGroupId??''),String(row.searchTerm??'')])
function positiveKeywordCoverage(row,keywords){
 return keywords.find(keyword=>keyword.matchType==='EXACT'&&keyword.status!=='REMOVED'&&String(keyword.campaignId)===String(row.campaignId)&&String(keyword.adGroupId)===String(row.adGroupId)&&text(keyword.keyword)===text(row.searchTerm))||null
}
function unavailableQuality(rows){
 const {totalSpend}=classifyQuality(rows)
 return{relevantClickRate:null,relevantSpendRate:null,wasteRate:null,classifiedSpendCoverage:null,classifiedClicks:null,classifiedSpend:null,totalSpend}
}
...
export function createWorkspaceService(d){
 ...
 async function readNegatives(args={}){
  const campaignFilter=args.campaignId?` AND campaign.id = ${requireId(args.campaignId)}`:'',groupFilter=`${campaignFilter}${args.adGroupId?` AND ad_group.id = ${requireId(args.adGroupId)}`:''}`
  const sources=await Promise.all([
   d.query(`SELECT campaign.id, campaign.name, campaign_criterion.status, campaign_criterion.keyword.text, campaign_criterion.keyword.match_type FROM campaign_criterion WHERE campaign_criterion.negative = TRUE AND campaign_criterion.type = 'KEYWORD' AND campaign_criterion.status != 'REMOVED'${campaignFilter} LIMIT 10000`),
   d.query(`SELECT campaign.id, campaign.name, ad_group.id, ad_group.name, ad_group_criterion.status, ad_group_criterion.keyword.text, ad_group_criterion.keyword.match_type FROM ad_group_criterion WHERE ad_group_criterion.negative = TRUE AND ad_group_criterion.type = 'KEYWORD' AND ad_group_criterion.status != 'REMOVED'${groupFilter} LIMIT 10000`),
   d.query(`SELECT campaign.id, campaign.name, campaign_shared_set.shared_set, campaign_shared_set.status FROM campaign_shared_set WHERE campaign_shared_set.status = 'ENABLED'${campaignFilter} LIMIT 10000`),
   d.query(`SELECT customer_negative_criterion.negative_keyword_list.shared_set FROM customer_negative_criterion WHERE customer_negative_criterion.type = 'NEGATIVE_KEYWORD_LIST' LIMIT 10000`),
   d.query(`SELECT shared_set.resource_name, shared_set.name, shared_set.type, shared_criterion.keyword.text, shared_criterion.keyword.match_type FROM shared_criterion WHERE shared_set.status = 'ENABLED' AND shared_criterion.type = 'KEYWORD' LIMIT 10000`)
  ])
  const[campaign,group,links,accountLinks,criteria]=sources
  ...
  return{negativeKeywords:result.filter(n=>n.keyword),complete:sources.every(rows=>rows.length<10000)}
 }
 async function getNegatives(args={}){
  const result=await readNegatives(args)
  if(!result.complete)throw new Error('The 10,000-row negative inventory limit was reached. Narrow the campaign or ad-group scope before live actions.')
 ...
 async function keywordLabSnapshot(args={}){
   ...
   const classificationEnabled=typeof d.db.searchTermClassifications==='function'
   ...
   const requests=[d.query(inventoryQuery),d.query(metricsQuery),d.query(optionalQuery),d.query(termsQuery)]
   if(classificationEnabled)requests.push(d.db.searchTermClassifications({campaignId:args.campaignId,adGroupId:args.adGroupId}),readNegatives(args))
   const [inventoryResult,metricsResult,optionalResult,termResult,classificationResult,negativeResult]=await Promise.allSettled(requests)
  ...
  if(metricsTruncated)warnings.push({section:'keyword metrics',message:'The 10,000-row keyword metrics limit was reached. Narrow the campaign or ad-group scope.'})
   ...
   const classifications=classificationEnabled&&classificationResult.status==='fulfilled'?classificationResult.value.map(normalizeClassification):[]
   const classificationInventoryComplete=!classificationEnabled||(classificationResult.status==='fulfilled'&&classifications.length<10000)
   if(classificationEnabled&&classificationResult.status==='rejected')warnings.push({section:'classifications',message:classificationResult.reason?.message||'Search-term classifications are unavailable.'})
   if(classificationEnabled&&classificationResult.status==='fulfilled'&&classifications.length>=10000)warnings.push({section:'classifications',message:'The 10,000-row classification inventory limit was reached. Narrow the campaign or ad-group scope.'})
   const negativeKeywords=classificationEnabled&&negativeResult.status==='fulfilled'?negativeResult.value.negativeKeywords:[]
   const negativeInventoryComplete=!classificationEnabled||(negativeResult.status==='fulfilled'&&negativeResult.value.complete)
   if(classificationEnabled&&negativeResult.status==='rejected')warnings.push({section:'negative inventory',message:negativeResult.reason?.message||'Negative keyword inventory is unavailable.'})
   if(classificationEnabled&&negativeResult.status==='fulfilled'&&!negativeResult.value.complete)warnings.push({section:'negative inventory',message:'The 10,000-row negative inventory limit was reached. Narrow the campaign or ad-group scope before live actions.'})
  ...
  for(const row of optionalResult.status==='fulfilled'?optionalResult.value:[]){const identity=keywordIdentity(row),key=criterionIdentity(identity);if(key)optionalByIdentity.set(key,optionalKeywordMetrics(row))}
   let searchTerms=(termResult.status==='fulfilled'?termResult.value:[]).map(row=>{
  ...
  for(const row of searchTerms){const identity=row.triggeringKeywordResourceName?`resource:${row.triggeringKeywordResourceName}`:row.triggeringCriterionId?`criterion:${row.adGroupId}:${row.triggeringCriterionId}`:null;if(!identity)continue;if(!termCounts.has(identity))termCounts.set(identity,new Set());termCounts.get(identity).add(row.searchTerm)}
   let keywords=(inventoryResult.status==='fulfilled'?inventoryResult.value:[]).map(row=>{
   ...
   keywords=keywords.map(row=>({...row,revision:keywordRevisionOf(row)}))
   ...
   if(classificationEnabled){
    const classificationByIdentity=new Map(classifications.map(row=>[classificationIdentity(row,accountId),row]))
    searchTerms=searchTerms.map(row=>{
     const persisted=classificationByIdentity.get(classificationIdentity(row,accountId)),negativeCoverage=coveringNegative(row,negativeKeywords),keywordCoverage=positiveKeywordCoverage(row,keywords)
     return{...row,classification:persisted?.classification??null,classificationNote:persisted?.note??null,classificationId:persisted?.id??null,classificationUpdatedAt:persisted?.updatedAt??null,negativeCoverage,keywordCoverage}
    })
    const termsByKeyword=new Map()
    for(const row of searchTerms){const key=row.triggeringKeywordResourceName?`resource:${row.triggeringKeywordResourceName}`:row.triggeringCriterionId?`criterion:${row.adGroupId}:${row.triggeringCriterionId}`:null;if(!key)continue;if(!termsByKeyword.has(key))termsByKeyword.set(key,[]);termsByKeyword.get(key).push(row)}
    const qualityFor=classificationInventoryComplete&&negativeInventoryComplete?classifyQuality:unavailableQuality
    keywords=keywords.map(row=>({...row,...qualityFor(termsByKeyword.get(criterionIdentity(row))||[])}))
    quality=qualityFor(searchTerms)
   }
   const coverageComplete=inventoryComplete&&metricsResult.status==='fulfilled'&&!metricsTruncated&&termResult.status==='fulfilled'&&!termsTruncated&&negativeInventoryComplete&&classificationInventoryComplete
   return{ok:true,account:d.account,range,keywords,searchTerms,termGroups:groupTermsByKeyword(searchTerms),...(classificationEnabled?{negativeKeywords,quality,negativeInventoryComplete,classificationInventoryComplete}:{}),warnings,inventoryComplete,coverageComplete,liveActionsEnabled:inventoryComplete&&negativeInventoryComplete&&Boolean(d.canWrite),fetchedAt:new Date().toISOString()}
  }
 ...
 async function classifyTerm(args={}){
  const campaignId=requireId(args.campaignId,'Campaign ID'),adGroupId=requireId(args.adGroupId,'Ad group ID'),searchTerm=String(args.searchTerm??'')
  if(!searchTerm.trim())throw new Error('Search term is required.')
  const classification=String(args.classification??'').toLowerCase()
  if(!['relevant','irrelevant','observe','promoted'].includes(classification))throw new Error('Classification must be relevant, irrelevant, observe, or promoted.')
  const note=args.note===null||args.note===undefined||!String(args.note).trim()?null:String(args.note).trim()
  const identity={accountId,campaignId,adGroupId,searchTerm},existing=await d.db.findSearchTermClassification(identity)
  const row=existing
   ?await d.db.updateSearchTermClassification(existing.id,{classification,note})
   :await d.db.insertSearchTermClassification({account_id:accountId,campaign_id:campaignId,ad_group_id:adGroupId,search_term:searchTerm,classification,note,created_by:d.actor.id})
  return{ok:true,classification:normalizeClassification(row)}
 }
 ...
 return{...createKeywordWriteService(d,{getNegatives}),snapshot,keywordLabSnapshot,keywordExperiments,classifyTerm,keywordCandidates,keywordHistory,apply,verify,audience,suggest,compare,history:()=>d.db.history(),getAds,getNegatives}
}
