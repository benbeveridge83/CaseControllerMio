...
export function createWorkspaceService(d){
 ...
 async function keywordLabSnapshot(args={}){
  ...
  const keywords=(inventoryResult.status==='fulfilled'?inventoryResult.value:[]).map(row=>{
   ...
   const totals=metricsResult.status==='fulfilled'?(metricsByIdentity.has(key)?aggregateReportingMetrics(metricsByIdentity.get(key)):zeroReportingMetrics()):aggregateReportingMetrics([])
