...
export function createWorkspaceService(d){
 ...
 async function keywordLabSnapshot(args={}){
  ...
  const coreQuery=`SELECT ${KEYWORD_FIELDS}, ${METRICS} FROM keyword_view WHERE ${dateWhere(range)}${extra} AND ad_group_criterion.status != 'REMOVED' AND ad_group_criterion.negative = FALSE ORDER BY metrics.cost_micros DESC LIMIT 10000`
  ...
  const [coreResult,optionalResult,termResult]=await Promise.allSettled([d.query(coreQuery),d.query(optionalQuery),d.query(termsQuery)])
  const inventoryComplete=coreResult.status==='fulfilled'
  if(!inventoryComplete)warnings.push({section:'keyword inventory',message:coreResult.reason?.message||'Keyword inventory is unavailable.'})
  ...
  const keywords=(coreResult.status==='fulfilled'?coreResult.value:[]).map(row=>{
   const identity=keywordIdentity(row),key=criterionIdentity(identity),criterion=row.adGroupCriterion||{}
   return{...identity,keyword:criterion.keyword?.text||'',matchType:criterion.keyword?.matchType||'',status:criterion.status||'UNKNOWN',...reportingMetrics(row),...(optionalByIdentity.get(key)||optionalKeywordMetrics()),...withPerDayMetrics(reportingMetrics(row),range.days),searchTermCount:termCounts.get(key)?.size||0}
  })
  if(keywords.length>=10000)warnings.push({section:'keyword inventory',message:'The 10,000-row reporting limit was reached. Narrow the date range.'})
  if(searchTerms.length>=10000)warnings.push({section:'search terms',message:'The 10,000-row reporting limit was reached. Narrow the date range.'})
  return{ok:true,account:d.account,range,keywords,searchTerms,termGroups:groupTermsByKeyword(searchTerms),warnings,inventoryComplete,coverageComplete:inventoryComplete&&termResult.status==='fulfilled',liveActionsEnabled:inventoryComplete&&Boolean(d.canWrite),fetchedAt:new Date().toISOString()}
 }
 async function keywordExperiments(args={}){
  ...
  const results=experiments.map(experiment=>{
   ...
   const totals=inventoryComplete?aggregateReportingMetrics(rows):aggregateReportingMetrics([])
 ...
 return{snapshot,keywordLabSnapshot,keywordExperiments,apply,verify,audience,suggest,compare,history:()=>d.db.history(),getAds,getNegatives}
}
