...
export function createKeywordWriteService(d,{getNegatives}){
 ...
 function check(items,{keywords,negatives}){
  ...
  return items.map(item=>{
   ...
   if(item.criterionResourceName){original=keywords.find(k=>k.criterionResourceName===item.criterionResourceName&&k.campaignId===item.campaignId&&k.adGroupId===item.adGroupId)
    ...
    if(item.kind==='change_match_type'&&original.status!=='ENABLED')throw new Error('Only an enabled keyword can be replaced; add another match type for a paused keyword.')
   }
   if(!isStatus(item)){
 ...
 function steps(item){
  ...
  const create={...(campaign?{campaign:`customers/${accountId}/campaigns/${item.campaignId}`}:{adGroup:`customers/${accountId}/adGroups/${item.adGroupId}`,status:'ENABLED'}),negative:isNegative(item),keyword:{text:item.keyword,matchType:item.matchType}}
 ...
 async function readStep(item,name,resource,observeOnly=false){
  ...
  const match=name==='pause'?item.original.matchType:item.matchType,status=name==='pause'?'PAUSED':isStatus(item)?targetStatus(item):'ENABLED'
 ...
 async function activateExperiment(item,result){
  ...
  try{const saved=await d.db.updateKeywordExperiment(result.experiment.id,{state:'active',criterion_id:result.create.resourceName.split('~')[1],criterion_resource_name:result.create.resourceName,experiment_started_at:result.create.verifiedAt});if(!saved||saved.criterion_resource_name!==result.create.resourceName||saved.state!=='active')throw new Error('Experiment activation was not confirmed');result.experiment={...result.experiment,state:'persisted'}}catch(e){result.experiment={...result.experiment,state:'unverified',message:e.message}}
 }
 ...
 async function keywordApply(body={}){
  ...
  for(let index=0;index<items.length;index++){
   ...
   for(const step of steps(item)){
    ...
    r.state=itemState(r)
    if(!await persist(event.id,result)||current.state==='unverified'){halt=true;break}
