...
function directionKind(value){
 const s=lower(value)
 if(!s)return{direction:'mixed',mapped:false}
 if(s.includes('methodolog')||s==='review'||s==='consensus')return{direction:'methodology_only',mapped:true}
 if(s.includes('disfavor')||s.includes('harmful')||s.includes('adverse process')||s.includes('reduced private'))return{direction:'disfavors_shared',mapped:true}
 if(s.includes('neutral after')||s.includes('disappears after'))return{direction:'neutral',mapped:true}
 if(s.includes('concern')||s.includes('conditional')||s.includes('eroded')||s.includes('reverse benefit'))return{direction:'conditional_concern',mapped:true}
 if(s.includes('favor')||s.includes('protective')||s.includes('least difficulties')||s.includes('sole worse')||s.includes('shared ~= intact'))return{direction:'favors_shared',mapped:true}
 if(s.includes('mixed')||s.includes('inconclusive')||s.includes('varied')||s.includes('does not show')||s.includes('mechanism/economic')||s.includes('economic/maternal'))return{direction:'mixed',mapped:true}
 return{direction:'mixed',mapped:false}
}
export function mapDirection(value){return directionKind(value).direction}
...
export function parseResearchInventory(workbook){
 ...
 const publications=rows(workbook,'Master Works').map(r=>{
  const mappedDirection=directionKind(r['Preliminary Direction']),source_type=mapSourceType(r['Source Type']),import_warnings=[]
  if(!mappedDirection.mapped)import_warnings.push(`Unmapped direction: ${clean(r['Preliminary Direction'])||'[blank]'}`)
  return{inventory_work_id:clean(r['Work ID']),title:clean(r['Title']),authors_text:clean(r['Authors']),publication_year:numOrNull(r['Year']),source_type,admission_route:clean(r['Admission Route'])||null,direct_child_outcome:bool(r['Direct Child Outcome?']),included_nielsen_2018:bool(r['Included in Nielsen 2018?']),country_text:clean(r['Country'])||null,doi:clean(r['DOI'])||null,finding_direction:mappedDirection.direction,editorial_status:'draft',citation_verification:clean(r['Citation Verification'])||null,internal_notes:clean(r['Notes'])||null,inventory_raw:{source_type_original:r['Source Type']??null,direction_original:r['Preliminary Direction']??null,sample_n:r['Sample N']??null,shared_equal_definition:r['Shared/Equal Definition']??null,design:r['Design']??null,access_status:r['Access Status']??null,import_warnings}}
 })
 ...
 const metrics=[]
 for(const r of rows(workbook,'Metric Snapshots')){
  const work=clean(r['Work ID']),value=numOrNull(r['Value'])
  if(!workIds.has(work)){warnings.push(`Metric row references missing Work ID ${work}`);continue}
  if(value===null)continue
  const retrieved=clean(r['Retrieved']),captured_at=dateOrNull(r['Retrieved'])
  if(!captured_at){if(retrieved)warnings.push(`Invalid metric date for ${work}: ${r['Retrieved']}`);else warnings.push(`Missing metric date for ${work}`);continue}
  metrics.push({inventory_work_id:work,provider:clean(r['Source'])||'inventory',metric_type:clean(r['Metric'])||'metric',metric_value:value,source_url:null,captured_at,notes:clean(r['Notes'])||null})
 }
 const studies=[]
