...
export async function saveResearchPublication(client,input){const row=cleanPublication(input);return fail(await client.from('research_publications').upsert(row,{onConflict:row.id?'id':'slug'}).select(PUBLICATION_COLUMNS).single())}
