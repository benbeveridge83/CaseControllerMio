...
export default function stickyFilters(){return{name:'mio-v312-sticky-filters',enforce:'pre',transform(source,id){
 ...
 // The Calendar's case/matter status matching (including the hide-closed default)
 // now lives in the tested module src/mioCalendarEventFilters.js.
 ...
 code=once(code,'setCalendarMatterStatusFilter(null)}>All',"setCalendarMatterStatusFilter(['__all__'])}>All",'Calendar matter All')
 code=once(code,'checked={!Array.isArray(calendarMatterStatusFilter) || calendarMatterStatusFilter.includes(value)}','checked={caseFilterMatches(calendarMatterStatusFilter,value)}','Calendar matter checkboxes')
 code=once(code,"const all = calendarStatusOptions('matter_status'); const selected = Array.isArray(current) ? current : all;","const all = calendarStatusOptions('matter_status'); const selected = caseFilterValues(current,all);",'Calendar matter toggle')
 code=once(code,"Array.isArray(calendarMatterStatusFilter) ? calendarMatterStatusFilter.length : calendarStatusOptions('matter_status').length","caseFilterValues(calendarMatterStatusFilter,calendarStatusOptions('matter_status')).length",'Calendar matter count')
