...
await context.route('**/*',async route=>{
 ...
 const tables={matters,clients:matters.map(m=>m.clients),mio_invoices:invoices,mio_invoice_events:events,lawpay_transactions:transactions,lawpay_payment_requests:paymentRequests,mio_billing_entries:[]}
...
try{
 ...
 assert.equal(await reconciliation.getByText(/PNC-CONSULT-/).count(),0,'a linked consultation payment is not reported as a missing invoice')
 assert.equal(await reconciliation.getByText(/The referenced Mio invoice was not found/).count(),0,'a consultation payment does not require an invoice')
 ...
 assert.equal(await reconciliation.getByText(/- Trust - trust deposit - Bravo Matter/).count(),0,'a posted trust deposit must leave the unlinked list')
 ...
 // The posted row is gone, so there is no "Categorize" button left to retry from the
 // reconciliation. The duplicate-posting refusal stays covered by the SQL behavior suite.
 assert.equal(await reconciliation.getByRole('button',{name:/^Categorize the .*Bravo Synthetic$/}).count(),0,'a posted charge has no categorize button left')
 assert.equal(gatewayState.classifications.filter(record=>String(record.gateway_transaction_id)==='provider-b').length,1,'no duplicate classification may exist')
 assert.equal(serverEffect('provider-b').length,1,'no duplicate ledger effect may exist')
 ...
 await reconciliation.locator('summary').filter({hasText:'2 unlinked transaction(s)'}).waitFor()
 ...
 await reconciliation.locator('summary').filter({hasText:'1 unlinked transaction(s)'}).waitFor()
 ...
 await reconciliation.locator('summary').filter({hasText:'1 unlinked transaction(s)'}).waitFor()
 assert.match(await reconciliation.getByRole('status').innerText(),/0 invoice issue\(s\); 1 unlinked transaction\(s\) require review/,'the scan message uses the same filtered count as the header')
 ...
 console.log(JSON.stringify({ok:true,checks:checks.length,financialWrites:writes.length,externalCalls:blocked.length,tests:['minimum trust boundary','review opens without send','existing draft reuse','sent-request duplicate prevention','status plus date sorting','paid oldest-first preset','collapsed finance settings','reconciliation shows the deposit account','categorize an unlinked charge as a matter trust deposit through the shared gateway','a posted charge leaves the unlinked list and count','an operating charge never moves trust','neither decision posts nothing','an unresolved deposit account is saved for review and never posted','operating ledger no trust movement']},null,2))
