...
try{
 ...
 await page.getByLabel('Sort',{exact:true}).selectOption('oldest');assert.deepEqual(await names(),['Beta matter','Alpha matter','Gamma matter'])
 ...
 await page.getByLabel('Status',{exact:true}).selectOption('waiting');await page.getByLabel('Update / evidence explanation').fill('Synthetic waiting update');await page.getByLabel('Waiting on',{exact:true}).fill('Client');await page.getByLabel('Follow-up due').fill(dateInput(future(2)))
 ...
 await page.getByLabel('File to customize',{exact:true}).selectOption('word-b')
 ...
 await page.getByLabel('File to customize',{exact:true}).selectOption('word-a');await page.getByRole('button',{name:'Signature block (customized)',exact:true}).click();assert.match(await page.locator('.mio-component-paper').innerText(),/Custom Attorney/);await page.screenshot({path:'test-results/drafting-component-preview.png'});await page.getByRole('button',{name:'Close preview',exact:true}).click()
 await page.getByRole('button',{name:'Drafting settings',exact:true}).click();await page.getByRole('heading',{name:'Case type connections and reusable components',exact:true}).waitFor();await page.getByLabel('Modification',{exact:true}).selectOption('divorce-test');await page.getByRole('button',{name:'Save shared defaults',exact:true}).click();await page.getByText('Saved to Supabase',{exact:true}).first().waitFor();assert.equal(JSON.parse(states.get('caseMioDraftingProfile').raw_value).case_type_style_map.Modification,'divorce-test');await page.screenshot({path:'test-results/drafting-bottom-settings.png'})
