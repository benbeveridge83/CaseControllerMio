...
try{
 ...
 console.log('PASS inline expansion; note and pause persistence; blocked paused actions')
 await draft().getByLabel('Connected drafting template',{exact:false}).selectOption('template-test')
 await settle(()=>workflows.get(ids[0]).state.steps.drafting.template_id==='template-test')
 await draft().getByLabel('Saved documents to review (select one or more)',{exact:false}).selectOption('pdf-a')
 assert.doesNotMatch(await draft().getByLabel('Saved documents to review (select one or more)',{exact:false}).innerText(),/Beta-private/)
 ...
 console.log('PASS same-matter document connections and time entry')
 ...
 await page.getByLabel('I reviewed the actual evidence and confirm completion.',{exact:false}).check()
 ...
 await filing().getByLabel('Reviewed PDF for e-filing',{exact:false}).selectOption('pdf-a')
 assert.doesNotMatch(await filing().getByLabel('Reviewed PDF for e-filing',{exact:false}).innerText(),/Beta-private/)
 ...
 console.log('PASS connected template generation; explicit approval; PDF eFile handoff without submission; no case-data browser storage')
