...
const docs=[{id:'pdf-a',matter_id:ids[0],file_name:'Alpha-reviewed-motion.pdf',file_type:'application/pdf',file_path:'synthetic/alpha-reviewed.pdf'},{id:'pdf-b',matter_id:ids[1],file_name:'Beta-private-motion.pdf',file_type:'application/pdf'}]
...
try{
 ...
 await page.getByLabel('Case type',{exact:false}).selectOption('Divorce')
 await page.getByLabel('Specific matter',{exact:false}).selectOption(ids[0])
 await page.getByRole('button',{name:'Hide graph',exact:true}).click()
 console.log('PASS graph case-type and matter filters')
 ...
 const detail=()=>page.locator('.mio-wd-expanded'),draft=()=>detail().locator('.mio-wd-step').filter({hasText:'Prepare and approve motion / order'}),focus=()=>detail().locator('.mio-wd-step-focus')
 ...
 await focus().getByRole('heading',{name:/Prepare and approve motion \/ order/}).waitFor()
 assert.match(await focus().innerText(),/next step Mio needs you to address/i)
 assert.equal(await focus().getByRole('button',{name:'Already completed',exact:true}).count(),1)
 await focus().getByRole('button',{name:'Add note',exact:true}).click()
 await page.getByLabel('Step note',{exact:false}).fill('Synthetic inline note - no progress change')
 ...
 assert.equal(await focus().getByRole('button',{name:'Yes, draft using this template',exact:true}).isDisabled(),true)
 ...
 await focus().getByRole('heading',{name:/Prepare and approve motion \/ order/}).waitFor()
 console.log('PASS compact expansion; selected-step panel; note and pause persistence; blocked paused actions')
 await focus().getByLabel('Connected drafting template',{exact:false}).selectOption('template-test')
 await settle(()=>workflows.get(ids[0]).state.steps.drafting.template_id==='template-test')
 await focus().getByLabel('Saved documents to review (select one or more)',{exact:false}).selectOption('pdf-a')
 assert.doesNotMatch(await focus().getByLabel('Saved documents to review (select one or more)',{exact:false}).innerText(),/Beta-private/)
 await detail().getByText('Billing entries — All withdrawal time entries (1)',{exact:true}).waitFor()
 ...
 await focus().getByRole('button',{name:/Add time/,exact:false}).click()
 ...
 await detail().getByText('Billing entries — All withdrawal time entries (2)',{exact:true}).waitFor()
 console.log('PASS same-matter document connections and visible billing entries')
 await focus().getByRole('button',{name:'Yes, draft using this template',exact:true}).click()
 ...
 await focus().getByRole('heading',{name:/Prepare and approve motion \/ order/}).waitFor()
 await focus().getByRole('button',{name:'Motion / order reviewed and approved',exact:true}).click()
 ...
 console.log('PASS connected template generation; simple completion; PDF eFile prompt/handoff without submission; no case-data browser storage')
