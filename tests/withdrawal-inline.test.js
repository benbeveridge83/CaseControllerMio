...
test('all configured transforms compose with inline detail, document handoff and billing callbacks',async()=>{
 ...
 for(const f of ['App.jsx','MioWithdrawalDashboard.jsx','mioWithdrawalRepository.js']){
  ...
  if(f==='App.jsx'){assert.match(source,/Mio V304/);assert.match(source,/mioWdPrepareEfile/);assert.match(source,/Return to withdrawal row/);assert.match(source,/matter_step:\"Withdrawal - \"/)}
  ...
  if(f==='mioWithdrawalRepository.js')assert.match(source,/mioWithdrawalWorkspaceState/)
 }
})
