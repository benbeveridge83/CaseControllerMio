...
try{
 ...
 await migration.page.locator('[data-mio-cloud-phase="ready"][data-mio-cloud-pending="0"]').waitFor({timeout:45000})
 ...
 await migration.page.reload();await migration.page.locator('[data-mio-cloud-phase="ready"][data-mio-cloud-pending="0"]').waitFor({timeout:45000});await migration.context.close()
 ...
 await big.page.locator('[data-mio-startup="loading"]').waitFor({timeout:30000})
 assert.match(await big.page.getByRole('status').innerText(),/Checking your sign-in session|Reading your saved records|Loading your saved records/)
 assert.equal(await big.page.getByText('Supabase').count(),0,'startup text must not name the storage backend')
 assert.equal(await big.page.getByText('Cloud startup').count(),0,'startup diagnostics must stay hidden from customers')
 await big.page.screenshot({path:'test-results/cloud-startup-progress.png'})
 await big.page.locator('[data-mio-cloud-phase="ready"][data-mio-cloud-pending="0"]').waitFor({timeout:90000})
