...
test('inventory parser preserves direction, zero metrics, and verified study detail',()=>{
 ...
 XLSX.utils.book_append_sheet(wb,sheet([['Work ID','Authors','Year','Title','Source Type','Admission Route','Direct Child Outcome?','Included in Nielsen 2018?','Preliminary Direction','Country','Sample N','Shared/Equal Definition','Design','DOI','Canonical URL','Access Status','Full Text / OA URL'],['W1','A',2025,'Good','Empirical study','x','Yes','No','Favors shared','US',100,'JPC >=35%','Cross-sectional','10/x','https://doi.org/10/x','OA','https://oa/1'],['W2','B',2024,'Concern','Empirical study','x','Yes','No','Disfavors joint-custody-law exposure','US',200,'50/50','Natural experiment','10/y','https://doi.org/10/y','Paywalled',''],['W3','C',2023,'Uncoded','Empirical study','x','Yes','No','Not independently coded','US',50,'JPC','Cross-sectional','','','','']]),'Master Works')
 ...
 XLSX.utils.book_append_sheet(wb,sheet([['Work ID','Metric','Value','Source','Retrieved','Notes'],['W1','citation_count',0,'OpenAlex','2026-09-12','n'],['W2','publisher_views',5,'Publisher','not-a-date','bad date']]),'Metric Snapshots')
 ...
 assert.equal(p.publications.length,3)
 assert.equal(p.publications[1].finding_direction,'disfavors_shared')
 assert.equal(p.publications[2].finding_direction,'mixed')
 assert.ok((p.publications[2].inventory_raw.import_warnings||[]).some(x=>x.includes('Unmapped direction')))
 assert.equal(p.metrics.length,1)
 assert.equal(p.metrics[0].metric_value,0)
 assert.ok(p.warnings.some(x=>x.includes('Invalid metric date for W2')))
