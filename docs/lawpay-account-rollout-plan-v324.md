...
* **Changes:** one function, `public.mio_reresolve_lawpay_accounts_v324(text,text,text)`. It restates
  ...
  data alone. It also refuses to write anything unless a named actor and an ACTIVE, exactly matching row in `mio_lawpay_accounts` both exist. The matching row is selected FOR SHARE and locked until the transaction ends, so a concurrent mapping change is either re-checked before the write or waits for it.
