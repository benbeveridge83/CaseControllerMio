...
1. `docs/lawpay-account-preflight-readonly.sql` (paste as its own query). It must report:
   ...
   * which V323/V324 objects are already present — this decides step 2;
...
## Step 2 — apply the V323 migration, only if the preflight says the objects are ABSENT
...
* **If the preflight says the objects are PRESENT:** skip this step entirely.
