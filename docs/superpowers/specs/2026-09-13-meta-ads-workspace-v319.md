# Meta Ads Workspace V319 — Design
...
- V318 is reserved for the Google Keyword Lab work on `feat/keyword-lab-v318`.
- This Meta/Facebook work is V319 on `feat/meta-ads-workspace-v319`.
- Do not edit, merge, rebase, or deploy the Keyword Lab branch from this workstream.
- Before final merge, whichever feature lands second must be updated against the then-current `main`, conflicts resolved, and its full tests rerun.
...
Mio should allow the user to draft all four before anything is written. The review screen should clearly separate resources that will be created, resources that already exist and will be reused, exact settings and copy, media selected/uploaded, budget and schedule, and targeting/placements.
...
Mio should distinguish token/account states:
...
Write controls remain disabled unless required permissions are confirmed.
...
`mutate` accepts only constrained actions such as:
...
8. Server re-reads affected resources.
...
Video processing is asynchronous. The workflow must use states such as uploading, processing, ready, and failed. Mio should not allow final ad creation until Meta reports the video ready.
...
AI may inspect campaign/ad set/ad performance, creative copy, objective, placements, device/platform breakdowns, region and demographic data where Meta/API-permissibly reports it, frequency/fatigue indicators, and lead signals.
...
Result states: verified, failed, skipped, unverified.

## Error Handling

Show Meta user-facing API errors whenever available, but do not leak tokens or secrets. Normalize authentication/token expired, missing permission, policy restriction, invalid targeting/objective combination, invalid creative/media, duplicate/idempotency conflict, stale resource, Meta transient/server error, and post-write verification mismatch.

No failed write should be silently retried if retrying could duplicate a campaign/ad set/ad.

## Testing

Unit/server tests must cover permission gating, firm-user authentication, mutation action allowlist, payload validation, idempotency behavior, normalization of Meta errors, immutable creative replacement behavior, post-write verification logic, and audit log creation.

UI tests must cover read-only mode, create flow draft -> review -> authorization, pause/resume, budget editing, image selection/upload, video processing state, before/after comparison, and failed/unverified outcomes.
...
3. Use a harmless or paused test resource for the first mutation when available.
...
1. Build and test on `feat/meta-ads-workspace-v319`.
...
8. Update against current `main` after the Keyword Lab work lands if necessary.
9. Resolve any overlap (especially the Marketing/App integration seam), rerun all V319 tests plus the relevant Google Ads regression tests, then merge only after verification passes.

## Success Criteria

V319 is successful when the user can open Mio and, without using Meta Ads Manager for ordinary work:
