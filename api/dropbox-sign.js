...
export default async function handler(req,res){
 ...
 try{
  if(req.method==='GET'){
   ...
   if(action==='status'){const data=await call('/account');return json(res,200,{connected:true,account_email:data?.account?.email_address||'',account_id:data?.account?.account_id||''})}
  ...
  if(action==='send'){
   ...
   return json(res,200,{signature_request:data.signature_request})
  }
  if(action==='cancel'){
