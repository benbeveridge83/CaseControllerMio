export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "create_matter_balance_all_dates") {
      ...
      return res.status(result.ok ? 200 : 422).json({ version: "v36", ...result });
    }

    if (action === "download") {
      const id = String(req.query.id || "").trim();
      if (!id) return res.status(400).json({ version: "v36", error: "Missing report id." });
      ...
      return res.status(502).json({ version: "v36", error: "Clio report download returned no CSV text or URL.", payload });
    }
    ...
    return res.status(200).json({ version: "v36", reports, trust_reports: trustReports, matter_balance_reports: matterBalanceReports, accounts_receivable_reports: accountsReceivableReports, report_count: reports.length });
  } catch (error) {
    return res.status(error.status || 500).json({ version: "v36", error: error.message || String(error), payload: error.payload || null, url: error.url || null });
  }
}
