export default async function handler(req, res) {
  ...
  function asDate(day) { const d = new Date(day); return Number.isFinite(d.getTime()) ? d : null; }
  function inRange(day) {
    const d = asDate(day); const from = fromDay ? asDate(fromDay) : null; const to = toDay ? asDate(toDay) : null;
  ...
  function firstNumber(...values) { for (const value of values) { const n = numberOrNull(value); if (n !== null) return n; } return null; }
  function uniqByDateKeepLast(points) { const map = new Map(); for (const p of points) if (p?.date) map.set(String(p.date).slice(0, 10), p); return Array.from(map.values()).sort((a,b)=>new Date(a.date)-new Date(b.date)); }
  ...
  async function clioFetch(url) {
    ...
    if (!response.ok) { const error = new Error(data?.error?.message || data?.message || data?.error || `Clio request failed with ${response.status}`); error.status = response.status; error.payload = data; throw error; }
    return data;
  }
  async function fetchPaged(path, params) {
    ...
    const all = [];
    while (url) { const data = await clioFetch(url); all.push(...(Array.isArray(data.data) ? data.data : [])); const next = data?.meta?.paging?.next; url = next ? new URL(next) : null; }
    return all;
  }
  ...
  async function fetchMatter(matterId) {
    const fieldSets = [
      ...
      "id,display_number,description,status,client{id,name},work_in_progress,work_in_progress_balance,wip,unbilled_balance,trust_balance,trust_account_balance",
      "id,display_number,description,status,client{id,name}",
    ];
    let merged = null;
    for (const fields of fieldSets) {
      const url = new URL(`https://app.clio.com/api/v4/matters/${matterId}`); url.searchParams.set("fields", fields);
      try { const data = await clioFetch(url); merged = { ...(merged || {}), ...(data?.data || {}) }; } catch {}
    }
    return merged;
  }
  ...
  function numericFromAccountBalances(matter, wantedType) {
    ...
    const rows = Array.isArray(accountBalances) ? accountBalances : Object.entries(accountBalances || {}).map(([key, value]) => ({ key, ...(typeof value === "object" ? value : { value }) }));
  ...
  async function fetchTrustTransactions(matterId) {
    const txs = await fetchPaged("bank_transactions.json", { limit: 200, matter_id: matterId, type: "liability", fields: "id,date,funds_out,funds_in,amount,running_balance,current_account_balance,matter{id,display_number}" }).catch(() => []);
    ...
    for (const tx of txs) {
      ...
      else if (fundsIn || fundsOut) running += fundsIn - fundsOut;
      else if (amount !== null) running += amount;
      else continue;
      const point = { date: day, trust: Number(running || 0), source: explicit !== null ? "clio_running_balance" : "computed_net_trust", transaction_id: tx.id };
      if (fromDay && asDate(day) < asDate(fromDay)) lastBeforeRange = point;
      if (inRange(day)) points.push(point);
    }
    if (fromDay && lastBeforeRange) points.unshift({ ...lastBeforeRange, date: fromDay, source: `${lastBeforeRange.source}_carried_to_start` });
  ...
  async function fetchCurrentWip(matterId, matter) {
    ...
    const activities = await fetchPaged("activities.json", { limit: 200, matter_id: matterId, type: "TimeEntry", fields: "id,date,total,non_billable,billed,bill{id,state,status},type" }).catch(() => []);
    ...
    for (const a of activities) {
      ...
      if (amount !== null) { total += amount; found = true; }
    }
  ...
  async function buildSeries(matterId) {
    ...
    const [trustPoints, wip] = await Promise.all([fetchTrustTransactions(matterId), fetchCurrentWip(matterId, matter)]);
    ...
    const points = requestedType === "wip"
      ...
      : rangedTrustPoints.map((p) => ({ date: p.date, balance: valueForType(requestedType, p.trust, wip, minimum), source: p.source, transaction_id: p.transaction_id }));
    return { matter_id: String(matterId), display_number: matter?.display_number || `Matter ${matterId}`, description: matter?.description || "", client_name: matter?.client?.name || "", account_type: requestedType, current_trust_balance: Number(currentTrust || 0), current_work_in_progress: Number(wip || 0), minimum_balance: minimum, points };
  }

  async function mapWithConcurrency(items, limit, mapper) {
    const results = []; let index = 0;
    async function worker() { while (index < items.length) { const currentIndex = index++; try { results[currentIndex] = { status: "fulfilled", value: await mapper(items[currentIndex]) }; } catch (error) { results[currentIndex] = { status: "rejected", reason: error }; } } }
  ...
  try {
    ...
    return res.status(200).json({ series, meta: { requested: matterIds.length, returned: series.length, failed: rejected.length, from: fromDay, to: toDay, mode: "restored_running_balance_points_and_current_wip" } });
  } catch (error) {
